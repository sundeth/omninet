...
CREATE INDEX ix_user_devices_secret_key ON user_devices (secret_key);
...
CREATE INDEX ix_game_modules_name ON game_modules (name);

-- module_contributors
CREATE TABLE module_contributors (
    id          UUID PRIMARY KEY DEFAULT gen_random_uuid(),
...
CREATE INDEX ix_activity_logs_target_id     ON activity_logs (target_id);
CREATE INDEX ix_activity_logs_created_at    ON activity_logs (created_at);

-- shop_cosmetics
CREATE TABLE shop_cosmetics (
    id            UUID PRIMARY KEY DEFAULT gen_random_uuid(),
...
CREATE INDEX ix_shop_cosmetics_cosmetic_type ON shop_cosmetics (cosmetic_type);

-- shop_gameplay
CREATE TABLE shop_gameplay (
    id          UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    name        VARCHAR(200) NOT NULL,
    description TEXT,
    price       INTEGER NOT NULL DEFAULT 0,
...
CREATE INDEX ix_user_purchases_user_id        ON user_purchases (user_id);
CREATE INDEX ix_user_purchases_item_id        ON user_purchases (item_id);
CREATE INDEX ix_user_purchases_purchase_type  ON user_purchases (purchase_type);
"""
